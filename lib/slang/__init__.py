__all__ = ['data', 'features', 'tokenize']
...
def find_token(token, spelling):
  for t in fwdfrom(token):
    if t.spelling == spelling:
      return t
  return None
cindex.Token.find = find_token

def rfind_token(token, spelling):
  for t in revfrom(token):
    if t.spelling == spelling:
      return t
  return None
cindex.Token.rfind = rfind_token
